...
function isUrlBlacklisted(url: string): boolean {
    try {
        ...
        console.warn(`解析 URL 时出错 (isUrlBlacklisted): ${url}`, error);
...
async function initializeUrlIndex() {
    try {
        ...
        // For DOCS_INDEX_NAME, use the imported getOrCreateIndex
        const docsIndex = await getOrCreateIndex(DOCS_INDEX_NAME); // Assumed to return Index<ArticleDocument>
        ...
        try {
            ...
            if (error.cause?.code === "index_not_found") {
                ...
                // 下一行存在 TypeScript linter 错误：类型"MeiliSearch"上不存在属性"waitForTask"。
                // 如果此错误导致脚本无法执行，请确保 MeiliSearch SDK 和其类型定义是最新的，
                // 或者作为临时方案使用类型断言：await (client as any).waitForTask(task.taskUid);
                ...
                console.error(
                    `检查或创建 URL 追踪索引 ${URL_INDEX_NAME} 失败:`,
                    error,
                );
                throw error;
            }
...
async function crawlPage(
    ...
    try {
        ...
        // 提取页面标题
        const title =
            document.querySelector("title")?.textContent || "未知标题";
        ...
        linkElements.forEach((link: Element) => {
            ...
            if (
                ...
                try {
                    ...
                    // 可选：记录无效的 URL
                    // console.warn(`在页面 ${url} 上解析链接 "${href}" 失败:`, e);
                }
            }
        });
...
async function savePage(pageData: PageData): Promise<boolean> {
    try {
        // 1. 保存到文档索引，使用 addDocumentsWithRetry 函数
        await addDocumentsWithRetry([
            {
                id: parseInt(pageData.id.substring(0, 8), 16) || 0, // 使用 MD5 的前 8 位转为数字作为 ID
                url: pageData.url,
                title: pageData.title,
                content: pageData.content,
                publishTimestamp: Math.floor(Date.now() / 1000),
                publishDateStr: new Date().toISOString(),
                author: "Apollo 文档爬虫",
                views: 0,
                likes: 0,
            } as ArticleDocument,
...
async function restoreQueueFromIndex(): Promise<string[]> {
    try {
        ...
        const queuedUrls = response.hits.map(
            (hit: CrawledUrlRecord) => hit.url,
        );
...
async function startCrawler() {
    ...
    try {
        ...
        while (urlQueue.length > 0) {
            ...
            try {
                // Linter 提示 getDocument 返回 Record<string, any>，我们断言其为 CrawledUrlRecord
                // 如果文档实际结构与 CrawledUrlRecord 不符且被找到，后续代码可能出错。
                // 但通常 MeiliSearch SDK 会正确返回泛型类型。
                const documentData = await urlIndexInstance.getDocument(urlId);
                docStatus = documentData as CrawledUrlRecord;
            } catch (e: any) {
                if (e.code === "document_not_found") {
        ...
        // 爬取完成后的统计数据
        const docsIndex = await getOrCreateIndex(DOCS_INDEX_NAME);
